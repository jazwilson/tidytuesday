...
```{r}
#Load library
pacman::p_load(inspectdf, visdat, readr, tidyverse,DataExplorer, tidyr, plyr,epiDisplay, showtext, ggplot2, gganimate,ggtext,av)
...
#Split the id column [year-place] into separate columns: [year] [place]
pk2 <- pk %>% separate(id, c("year", "place"), "-")
...
pk5 <- pk4 %>% subset(country %in% c("United States", "Canada", "Germany", "Italy", "Austria", "France", "Japan", "United Kingdom", "Finland", "The Netherlands"))
...
a <- animate(anim, height = 12, width = 15, units = "in", res = 150, renderer = av_renderer())
anim_save("Pumpkin_animation2.mp4", a)
anim_save("Pumpkin_animation3.gif", a)
```
